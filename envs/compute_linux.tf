...
resource "terraform_data" "remote_exec_oracle_mount_1" {
  depends_on = [
    time_sleep.wait_oracle_cloud_init_finish,
    oci_core_volume_attachment.attach_oracle_volume_para
  ]
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sleep 5",
      "sudo mkdir /mnt/para01",
      "sleep 5",
      "LINE=\"UUID=$(lsblk -no UUID /dev/sdb1)  /mnt/para01 xfs defaults,_netdev,nofail 0 2\"",
      "grep -q \"^UUID=$(lsblk -no UUID /dev/sdb1) \" /etc/fstab || echo \"$LINE\" | sudo tee -a /etc/fstab",
      "sudo systemctl daemon-reload",
      "sleep 5",
      "sudo mount -a",
      "sleep 5",
      "sudo chown opc:opc /mnt/para01"
    ]
  }
}
...
resource "terraform_data" "remote_exec_oracle_mount_2" {
  depends_on = [
    time_sleep.wait_oracle_cloud_init_finish,
    oci_core_volume_attachment.attach_oracle_volume_iscsi_by_agent,
    terraform_data.remote_exec_oracle_mount_1
  ]
  provisioner "remote-exec" {
    connection {
      agent       = false
    ...
    inline = [
      "sleep 5",
      "sudo parted -s -a optimal /dev/sdc mklabel gpt mkpart primary 0% 100%",
      "sleep 5",
      "sudo mkfs.xfs /dev/sdc1",
      "sleep 5",
      "sudo mkdir /mnt/iscsi01",
      "sleep 5",
      "LINE=\"UUID=$(lsblk -no UUID /dev/sdc1)  /mnt/iscsi01 xfs defaults,_netdev,nofail 0 2\"",
      "grep -q \"^UUID=$(lsblk -no UUID /dev/sdc1) \" /etc/fstab || echo \"$LINE\" | sudo tee -a /etc/fstab",
      "sudo systemctl daemon-reload",
      "sleep 5",
      "sudo mount -a",
      "sleep 5",
      "sudo chown opc:opc /mnt/iscsi01"
    ]
  }
}
...
resource "terraform_data" "remote_exec_oracle_mount_3" {
  depends_on = [
    time_sleep.wait_oracle_cloud_init_finish,
    oci_core_volume_attachment.attach_oracle_volume_iscsi_by_command,
    terraform_data.remote_exec_oracle_iscsi,
    terraform_data.remote_exec_oracle_mount_2
  ]
  provisioner "remote-exec" {
    connection {
      agent       = false
    ...
    inline = [
      "sleep 5",
      "sudo parted -s -a optimal /dev/sdd mklabel gpt mkpart primary 0% 100%",
      "sleep 5",
      "sudo mkfs.xfs /dev/sdd1",
      "sleep 5",
      "sudo mkdir /mnt/iscsi02",
      "sleep 5",
      "LINE=\"UUID=$(lsblk -no UUID /dev/sdd1)  /mnt/iscsi02 xfs defaults,_netdev,nofail 0 2\"",
      "grep -q \"^UUID=$(lsblk -no UUID /dev/sdd1) \" /etc/fstab || echo \"$LINE\" | sudo tee -a /etc/fstab",
      "sudo systemctl daemon-reload",
      "sleep 5",
      "sudo mount -a",
      "sleep 5",
      "sudo chown opc:opc /mnt/iscsi02"
    ]
  }
}
